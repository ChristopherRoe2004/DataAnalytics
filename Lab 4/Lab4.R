...
ggplot(scale(wine[,-13], scale = FALSE, center = TRUE), aes(x = Alcohol, 
  y = wine$Proline, colour = wine$Type_group)) +
  geom_point()

# Scatter plot for standardized data
ggplot(scale(wine[,-13], scale = TRUE, center = FALSE), aes(x = Alcohol, 
  y = wine$Proline, colour = wine$Type_group)) +
  geom_point()
...
cm_all <- table(Predicted = knn_pred_all, Actual = wine$Type_group, 
...
k <- 65
knn_pred_pca <- knn(train = wine_pca$x[,c(1,2)], test = wine_pca$x[,c(1,2)], 
                    cl = wine$Type_group, k = k)

# Confusion matrix for PCA (first 2 PCs)
cm_pca <- table(Predicted = knn_pred_pca, Actual = wine$Type_group, 
                dnn = list('predicted','actual'))
...
k <- 3
knn_pred_reduced <- knn(train = wine_pca_reduced$x[,1:3], test = wine_pca_reduced$x[,1:3], 
                        cl = wine$Type_group, k = k)

# Confusion matrix for reduced PCA
cm_reduced <- table(Predicted = knn_pred_reduced, Actual = wine$Type_group, 
                    dnn = list('predicted','actual'))
